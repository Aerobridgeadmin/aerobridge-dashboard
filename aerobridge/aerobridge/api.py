...
from aerobridge.aerobridge.doctype.course_lesson.course_lesson import save_progress
from aerobridge.aerobridge.utils import (
	LMS_ROLES,
	can_modify_batch,
	can_modify_course,
	get_average_rating,
...
def validate_billing_access(billing_type: str, name: str):
	doctype = "Aerobridge Batch" if billing_type == "batch" else "Aerobridge Course"
...
def verify_billing_access(doctype, name, billing_type):
	...
	if access and billing_type == "course":
		membership = frappe.db.exists("Aerobridge Enrollment", {"member": frappe.session.user, "course": name})
		if membership:
			access = False
			message = _("You are already enrolled for this course.")

	elif access and billing_type == "batch":
		membership = frappe.db.exists("Aerobridge Batch Enrollment", {"member": frappe.session.user, "batch": name})
		if membership:
			access = False
			message = _("You are already enrolled for this batch.")

		seat_count = frappe.get_cached_value("Aerobridge Batch", name, "seat_count")
		number_of_students = frappe.db.count("Aerobridge Batch Enrollment", {"batch": name})
		if seat_count <= number_of_students:
			access = False
			message = _("Batch is sold out.")

		start_date = frappe.get_cached_value("Aerobridge Batch", name, "start_date")
...
def get_chart_details():
	...
	details.enrollments = frappe.db.count("Aerobridge Enrollment")
	...
	details.completions = frappe.db.count("Aerobridge Enrollment", {"progress": ["like", "%100%"]})
	details.certifications = frappe.db.count("Aerobridge Certificate", {"published": 1})
	return details
...
def get_unsplash_photos(keyword: str = None):
	from aerobridge.unsplash import get_by_keyword, get_list
...
def get_certified_participant_details(member: str):
	count = frappe.db.count("Aerobridge Certificate", {"member": member})
...
def get_certification_query(filters: dict = None):
	Certificate = frappe.qb.DocType("Aerobridge Certificate")
...
def get_sidebar_settings():
	aerobridge_settings = frappe.get_single("Aerobridge Settings")
	if frappe.session.user == "Guest" and not aerobridge_settings.allow_guest_access:
		return []
	...
	for item in items:
		sidebar_items[item] = aerobridge_settings.get(item)

	if len(aerobridge_settings.sidebar_items):
		web_pages = frappe.get_all(
			"Aerobridge Sidebar Item",
			{"parenttype": "Aerobridge Settings", "parentfield": "sidebar_items"},
...
def update_sidebar_item(webpage: str, icon: str):
	...
	filters = {
		"web_page": webpage,
		"parenttype": "Aerobridge Settings",
		"parentfield": "sidebar_items",
		"parent": "Aerobridge Settings",
	}

	if frappe.db.exists("Aerobridge Sidebar Item", filters):
		frappe.db.set_value("Aerobridge Sidebar Item", filters, "icon", icon)
	else:
		doc = frappe.new_doc("Aerobridge Sidebar Item")
		doc.update(filters)
		doc.icon = icon
		doc.insert()
...
def delete_lesson(lesson: str, chapter: str):
	...
	frappe.db.delete("Aerobridge Course Progress", {"lesson": lesson})
...
def get_members(start: int = 0, search: str = None):
	...
	for member in members:
		...
		elif "Aerobridge Student" in roles:
			member.role = "Aerobridge Student"
...
def save_evaluation_details(
	...
	evaluation = frappe.db.exists("Aerobridge Certificate Evaluation", {"member": member, "course": course})
	...
	if evaluation:
		frappe.db.set_value("Aerobridge Certificate Evaluation", evaluation, details)
		return evaluation
	else:
		doc = frappe.new_doc("Aerobridge Certificate Evaluation")
		details.update(
			{
				"member": member,
				"course": course,
...
def save_certificate_details(
	...
	"""
	frappe.only_for(["Batch Evaluator", "Moderator"])
	certificate = frappe.db.exists("Aerobridge Certificate", {"member": member, "course": course})
	...
	if certificate:
		frappe.db.set_value("Aerobridge Certificate", certificate, details)
		return certificate
	else:
		doc = frappe.new_doc("Aerobridge Certificate")
		details.update(
			{
				"member": member,
				"course": course,
...
def update_course_statistics():
	courses = frappe.get_all("Aerobridge Course", fields=["name"])

	for course in courses:
		lessons = get_lesson_count(course.name)

		enrollments = frappe.db.count("Aerobridge Enrollment", {"course": course.name, "member_type": "Student"})
...
def get_announcements(batch: str):
	...
	is_batch_student = frappe.db.exists(
		"Aerobridge Batch Enrollment", {"batch": batch, "member": frappe.session.user}
	)
	...
	communications = frappe.get_all(
		"Communication",
		filters={
			"reference_doctype": "Aerobridge Batch",
...
def delete_course(course: str):
	...
	frappe.db.delete("Aerobridge Enrollment", {"course": course})
	frappe.db.delete("Aerobridge Course Progress", {"course": course})
	frappe.db.set_value("Aerobridge Quiz", {"course": course}, "course", None)
	frappe.db.set_value("Aerobridge Quiz Submission", {"course": course}, "course", None)
	...
	frappe.delete_doc("Aerobridge Course", course)
...
def delete_batch(batch: str):
	...
	frappe.db.delete("Aerobridge Batch Enrollment", {"batch": batch})
	frappe.db.delete("Batch Course", {"parent": batch, "parenttype": "Aerobridge Batch"})
	frappe.db.delete("Aerobridge Assessment", {"parent": batch, "parenttype": "Aerobridge Batch"})
	frappe.db.delete("Aerobridge Batch Timetable", {"parent": batch, "parenttype": "Aerobridge Batch"})
	frappe.db.delete("Aerobridge Batch Feedback", {"batch": batch})
	delete_batch_discussions(batch)
	frappe.db.delete("Aerobridge Batch", batch)


def delete_batch_discussions(batch: str):
	topics = frappe.get_all(
		"Discussion Topic",
		{"reference_doctype": "Aerobridge Batch", "reference_docname": batch},
		pluck="name",
	)
...
def give_discussions_permission():
	...
	roles = ["Aerobridge Student", "Course Creator", "Moderator", "Batch Evaluator"]
...
def update_document_details(notification: dict) -> dict:
	if notification.document_type == "Aerobridge Course":
		details = frappe.db.get_value(
			"Aerobridge Course", notification.document_name, ["title", "video_link", "short_introduction"], as_dict=1
		)
		instructors = get_instructors("Aerobridge Course", notification.document_name)
		details["instructors"] = instructors
		notification["document_details"] = details

	elif notification.document_type == "Aerobridge Batch":
		details = frappe.db.get_value(
			"Aerobridge Batch",
		...
		instructors = get_instructors("Aerobridge Batch", notification.document_name)
		details["instructors"] = instructors
		notification["document_details"] = details
	return notification
...
def get_aerobridge_settings():
	...
	for field in allowed_fields:
		settings[field] = frappe.get_cached_value("Aerobridge Settings", None, field)
...
def cancel_evaluation(evaluation: dict):
	...
	frappe.db.set_value("Aerobridge Certificate Request", evaluation.name, "status", "Cancelled")
...
def get_certification_details(course: str):
	...
	paid_certificate = frappe.db.get_value("Aerobridge Course", course, "paid_certificate")
...
def capture_user_persona(responses: str):
	...
	if response.get("message").get("name"):
		frappe.db.set_single_value("Aerobridge Settings", "persona_captured", True)
	return response
...
def make_new_exercise_submission(exercise: str, code: str, test_cases: list):
	submission = frappe.new_doc("Aerobridge Programming Exercise Submission")
...
def update_exercise_submission(submission: str, code: str, test_cases: list):
	member = frappe.db.get_value("Aerobridge Programming Exercise Submission", submission, "member")
	...
	frappe.db.set_value("Aerobridge Programming Exercise Submission", submission, {"status": status, "code": code})
...
def update_test_cases(test_cases: list, submission: str):
	frappe.db.delete("Aerobridge Test Case Submission", {"parent": submission})
	for row in test_cases:
		test_case = frappe.new_doc("Aerobridge Test Case Submission")
		test_case.update(
			{
				"parent": submission,
				"parenttype": "Aerobridge Programming Exercise Submission",
...
def track_video_watch_duration(lesson: str, videos: list):
	...
	for video in videos:
		...
		existing_record = frappe.db.get_value(
			"Aerobridge Video Watch Duration", filters, ["name", "watch_time"], as_dict=True
		)
...
def track_new_watch_time(lesson: str, video: dict):
	doc = frappe.new_doc("Aerobridge Video Watch Duration")
	doc.lesson = lesson
...
def get_pwa_manifest():
	...
	manifest = {
		...
		"start_url": get_aerobridge_route(),
		"icons": [
			{
				"src": banner_image or "/assets/aerobridge/frontend/manifest/manifest-icon-192.maskable.png",
				"sizes": "192x192",
				"type": "image/png",
...
def get_my_live_classes():
	...
	if len(live_class_details):
		for live_class in live_class_details:
			live_class.course_title = frappe.db.get_value("Aerobridge Course", live_class.course, "title")
...
def get_created_courses():
	...
	Course = frappe.qb.DocType("Aerobridge Course")
...
def get_created_batches():
	...
	CourseInstructor = frappe.qb.DocType("Course Instructor")
	Batch = frappe.qb.DocType("Aerobridge Batch")
...
def get_admin_live_classes():
	CourseInstructor = frappe.qb.DocType("Course Instructor")
	LMSLiveClass = frappe.qb.DocType("Aerobridge Live Class")
...
def get_admin_evals():
	...
	for evaluation in evals:
		evaluation.course_title = frappe.db.get_value("Aerobridge Course", evaluation.course, "title")
...
def delete_programming_exercise(exercise: str):
	frappe.only_for(["Moderator", "Course Creator", "Batch Evaluator"])
	frappe.db.delete("Aerobridge Programming Exercise Submission", {"exercise": exercise})
	frappe.db.delete("Aerobridge Programming Exercise", exercise)
...
def get_lesson_completion_stats(course: str):
	...
	CourseProgress = frappe.qb.DocType("Aerobridge Course Progress")
	...
	for quiz in quizzes:
		...
		if len(submissions):
			...
			attempts.append(
				{
					"quiz": quiz,
					"quiz_title": frappe.db.get_value("Aerobridge Quiz", quiz, "title"),
					"score": 0,
					"percentage": 0,
				}
			)
	...
	for assignment in assignments:
		...
		if len(assignment_subs):
			...
			submissions.append(
				{
					"assignment": assignment,
					"assignment_title": frappe.db.get_value("Aerobridge Assignment", assignment, "title"),
	...
	for exercise in exercises:
		...
		if len(exercise_subs):
			...
			submissions.append(
				{
					"exercise": exercise,
					"exercise_title": frappe.db.get_value("Aerobridge Programming Exercise", exercise, "title"),
