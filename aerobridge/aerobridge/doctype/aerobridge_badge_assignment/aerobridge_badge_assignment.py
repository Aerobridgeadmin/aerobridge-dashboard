...
from aerobridge.aerobridge.doctype.aerobridge_badge.aerobridge_badge import eval_condition
...
class LMSBadgeAssignment(Document):
	...
	def validate_owner(self):
		event = frappe.db.get_value("Aerobridge Badge", self.badge, "event")
	...
	def validate_duplicate_badge_assignment(self):
		grant_only_once = frappe.db.get_value("Aerobridge Badge", self.badge, "grant_only_once")
	...
	def validate_badge_criteria(self):
		badge_details = frappe.db.get_value(
			"Aerobridge Badge", self.badge, ["reference_doctype", "user_field", "condition", "enabled"], as_dict=True
		)
