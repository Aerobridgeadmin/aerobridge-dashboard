...
frappe.ui.form.on("Aerobridge Batch", {
	onload: function (frm) {
		...
		frm.set_query("assessment_type", "assessment", function () {
			let doctypes = ["Aerobridge Quiz", "Aerobridge Assignment"];
			return {
				filters: {
					name: ["in", doctypes],
				},
...
const set_timetable = (frm) => {
	if (frm.doc.timetable_template) {
		...
		frappe.call({
			...
			args: {
				doctype: "Aerobridge Batch Timetable",
				parent: "Aerobridge Timetable Template",
				fields: [
					"reference_doctype",
					"reference_docname",
					"day",
				...
				filters: {
					...
					parenttype: "Aerobridge Timetable Template",
				},
				order_by: "idx",
			},
			callback: (data) => {
...
const set_legends = (frm) => {
	if (frm.doc.timetable_template) {
		...
		frappe.call({
			method: "frappe.client.get_list",
			args: {
				doctype: "Aerobridge Timetable Legend",
				parent: "Aerobridge Timetable Template",
...
const set_default_legends = (frm) => {
	const data = [
		...
		{
			reference_doctype: "Aerobridge Quiz",
			label: "Aerobridge Quiz",
			color: "#39E4A5",
		},
		{
			reference_doctype: "Aerobridge Assignment",
			label: "Aerobridge Assignment",
			color: "#ECAD4B",
		},
		{
			reference_doctype: "Aerobridge Live Class",
			label: "Aerobridge Live Class",
			color: "#bb8be8",
		},
	];
