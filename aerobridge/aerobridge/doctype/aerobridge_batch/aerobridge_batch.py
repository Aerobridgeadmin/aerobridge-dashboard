...
class LMSBatch(Document):
	...
	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "Aerobridge Batch")
	...
	def validate_duplicate_courses(self):
		...
		if len(duplicates):
			title = frappe.db.get_value("Aerobridge Course", next(iter(duplicates)), "title")
	...
	def validate_seats_left(self):
		...
		students = frappe.db.count("Aerobridge Batch Enrollment", {"batch": self.name})
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("Aerobridge Batch", self.name)


def send_notification_for_published_batch(batch):
	send_notification = frappe.db.get_single_value("Aerobridge Settings", "send_notification_for_published_batches")
...
def send_email_notification_for_published_batch(batch):
	...
	instructors = get_instructors("Aerobridge Batch", batch.name)
	...
	args = {
		...
		"batch_url": frappe.utils.get_url(get_aerobridge_route(f"batches/{batch.name}")),
	}
	...
	frappe.db.set_value("Aerobridge Batch", batch.name, "notification_sent", 1)
	...
	notification = frappe._dict(
		{
			...
			"document_type": "Aerobridge Batch",
			"document_name": batch.name,
			"from_user": instructors[0] if instructors else None,
			"type": "Alert",
			"link": get_aerobridge_route(f"batches/{batch.name}"),
		}
	)
...
def authenticate(zoom_account):
	zoom = frappe.get_doc("Aerobridge Zoom Settings", zoom_account)
	if not zoom.enabled:
...
def get_batch_timetable(batch: str):
	...
	show_live_class = frappe.db.get_value("Aerobridge Batch", batch, "show_live_class")
	if show_live_class:
...
def get_live_classes(batch):
	...
	for class_ in live_classes:
		...
		class_.reference_doctype = "Aerobridge Live Class"
...
def get_timetable_details(timetable):
	for entry in timetable:
		...
		elif entry.reference_doctype == "Aerobridge Quiz":
			entry.url = "/quizzes"
			details = get_quiz_details(assessment, frappe.session.user)
			entry.update(details)

		elif entry.reference_doctype == "Aerobridge Assignment":
...
def send_batch_start_reminder():
	...
	for batch in batches:
		students = frappe.get_all("Aerobridge Batch Enrollment", {"batch": batch.name}, ["member", "member_name"])
...
def has_permission(doc, ptype="read", user=None):
	...
	is_enrolled = frappe.db.exists("Aerobridge Batch Enrollment", {"batch": doc.name, "member": user})
	if is_enrolled:
		return True

	is_batch_published = frappe.db.get_value("Aerobridge Batch", doc.name, "published")
