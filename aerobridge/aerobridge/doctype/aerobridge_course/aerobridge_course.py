...
class LMSCourse(Document):
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("Aerobridge Course", self.name)

	def send_email_to_interested_users(self):
		interested_users = frappe.get_all("Aerobridge Course Interest", {"course": self.name}, ["name", "user"])
		subject = self.title + " is available!"
		args = {
			"title": self.title,
			"course_link": get_aerobridge_route(f"courses/{self.name}"),
		...
		for user in interested_users:
			...
			email_args = frappe._dict(
				...
				template="aerobridge_course_interest",
				args=args,
				now=True,
			)
			frappe.enqueue(method=frappe.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			frappe.db.set_value("Aerobridge Course Interest", user.name, "email_sent", True)

	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "Aerobridge Course")
...
def send_notification_for_published_courses():
	send_notification = frappe.db.get_single_value("Aerobridge Settings", "send_notification_for_published_courses")
...
def send_email_notification_for_published_courses(courses):
	...
	for course in courses:
		instructors = get_instructors("Aerobridge Course", course.name)
		...
		args = {
			...
			"course_url": frappe.utils.get_url(get_aerobridge_route(f"courses/{course.name}")),
		}
		...
		frappe.db.set_value("Aerobridge Course", course.name, "notification_sent", 1)
		...
		notification = frappe._dict(
			{
				...
				"document_type": "Aerobridge Course",
				"document_name": course.name,
				"from_user": instructors[0] if instructors else None,
				"type": "Alert",
				"link": get_aerobridge_route(f"courses/{course.name}"),
			}
		)
