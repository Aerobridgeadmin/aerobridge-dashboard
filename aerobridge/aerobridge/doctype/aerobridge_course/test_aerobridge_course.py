...
from aerobridge.aerobridge.api import delete_course
from aerobridge.aerobridge.test_helpers import BaseTestUtils
...
class TestLMSCourse(BaseTestUtils):
	...
	def test_new_course(self):
		...
		self.assertTrue(frappe.db.exists("Aerobridge Course", course.name))
	...
	def test_delete_course(self):
		...
		self._create_user(user_email, "Test", "Member", ["Aerobridge Student"])
		...
		self.assertFalse(frappe.db.exists("Aerobridge Course", course.name))
		self.assertFalse(frappe.db.exists("Course Chapter", chapter.name))
		self.assertFalse(frappe.db.exists("Course Lesson", lesson.name))
		self.assertFalse(frappe.db.exists("Aerobridge Enrollment", enrollment.name))
		self.assertFalse(frappe.db.exists("Aerobridge Course Progress", {"course": course.name}))
		...
		self.cleanup_items.remove(("Aerobridge Course", course.name))
		self.cleanup_items.remove(("Aerobridge Enrollment", enrollment.name))
		self.cleanup_items.remove(("Aerobridge Course Progress", progress.name))
