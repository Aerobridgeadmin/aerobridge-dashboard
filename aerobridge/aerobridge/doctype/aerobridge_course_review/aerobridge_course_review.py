...
class LMSCourseReview(Document):
	...
	def validate_enrollment(self):
		enrollment = frappe.db.exists("Aerobridge Enrollment", {"course": self.course, "member": self.owner})
		if not enrollment:
			frappe.throw(_("You must be enrolled in the course to submit a review"))

	def validate_if_already_reviewed(self):
		if frappe.db.exists("Aerobridge Course Review", {"course": self.course, "owner": self.owner}):
