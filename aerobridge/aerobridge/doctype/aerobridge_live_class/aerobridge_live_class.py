...
from aerobridge.aerobridge.doctype.aerobridge_batch.aerobridge_batch import authenticate
...
class LMSLiveClass(Document):
	...
	def add_event_participants(self, event, calendar):
		participants = frappe.get_all("Aerobridge Batch Enrollment", {"batch": self.batch_name}, pluck="member")
		instructors = frappe.get_all(
			"Course Instructor", {"parenttype": "Aerobridge Batch", "parent": self.batch_name}, pluck="instructor"
		)
...
def create_attendance(live_class, data):
	for participant in data:
		doc = frappe.new_doc("Aerobridge Live Class Participant")
...
def update_attendees_count(live_class, data):
	frappe.db.set_value("Aerobridge Live Class", live_class.name, "attendees", len(data))
