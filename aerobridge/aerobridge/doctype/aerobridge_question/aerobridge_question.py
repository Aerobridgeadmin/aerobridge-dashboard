...
from aerobridge.aerobridge.utils import has_course_instructor_role, has_moderator_role
...
def update_question_title(question):
	if not question.is_new():
		question_rows = frappe.get_all("Aerobridge Quiz Question", {"question": question.name}, pluck="name")

		for row in question_rows:
			frappe.db.set_value("Aerobridge Quiz Question", row, "question_detail", question.question)
