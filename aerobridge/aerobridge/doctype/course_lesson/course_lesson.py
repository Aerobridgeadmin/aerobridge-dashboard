...
from aerobridge.aerobridge.utils import get_course_progress, recalculate_course_progress
...
class CourseLesson(Document):
	...
	def validate_quiz_id(self):
		if self.quiz_id and not frappe.db.exists("Aerobridge Quiz", self.quiz_id):
	...
	def save_lesson_details_in_quiz(self, content):
		...
		for block in content.get("blocks"):
			if block.get("type") == "quiz":
				...
				if not frappe.db.exists("Aerobridge Quiz", quiz):
...
def save_progress(lesson: str, course: str, scorm_details: dict = None):
	...
	"""
	membership = frappe.db.exists("Aerobridge Enrollment", {"course": course, "member": frappe.session.user})
	if not membership:
		return 0

	frappe.db.set_value("Aerobridge Enrollment", membership, "current_lesson", lesson)
	progress_already_exists = frappe.db.exists(
		"Aerobridge Course Progress", {"lesson": lesson, "member": frappe.session.user}
	)
	...
	enrollment = frappe.get_doc("Aerobridge Enrollment", membership)
...
def capture_progress_for_analytics():
	capture("course_progress", "aerobridge")
...
def get_quiz_progress(lesson):
	...
	for quiz in quizzes:
		passing_percentage = frappe.db.get_value("Aerobridge Quiz", quiz, "passing_percentage")
