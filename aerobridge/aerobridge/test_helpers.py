...
from aerobridge.aerobridge.doctype.aerobridge_certificate.aerobridge_certificate import get_default_certificate_template
...
class BaseTestUtils(UnitTestCase):
	...
	def _create_course(self, title="Utility Course", instructor="frappe@example.com"):
		existing = frappe.db.exists("Aerobridge Course", {"title": title})
		if existing:
			return frappe.get_doc("Aerobridge Course", existing)

		course = frappe.new_doc("Aerobridge Course")
		course.update(
			{
				"title": title,
		...
		course.save()
		self.cleanup_items.append(("Aerobridge Course", course.name))
		return course
	...
	def _create_enrollment(self, member, course):
		existing = frappe.db.exists("Aerobridge Enrollment", {"course": course, "member": member})
		if existing:
			return frappe.get_doc("Aerobridge Enrollment", existing)

		enrollment = frappe.new_doc("Aerobridge Enrollment")
		enrollment.update({"member": member, "course": course})
		enrollment.insert()
		self.cleanup_items.append(("Aerobridge Enrollment", enrollment.name))
		return enrollment

	def _create_progress(self, member, course, lesson):
		progress = frappe.new_doc("Aerobridge Course Progress")
		progress.update({"member": member, "course": course, "lesson": lesson})
		progress.insert()
		self.cleanup_items.append(("Aerobridge Course Progress", progress.name))
		return progress
	...
	def _create_batch(
		...
		existing = frappe.db.exists("Aerobridge Batch", {"title": title})
		if existing:
			return frappe.get_doc("Aerobridge Batch", existing)

		batch = frappe.new_doc("Aerobridge Batch")
		batch.update(
			{
				"title": title,
				"start_date": nowdate(),
		...
		batch.save()
		self.cleanup_items.append(("Aerobridge Batch", batch.name))
		return batch

	def _create_batch_enrollment(self, member, batch):
		existing = frappe.db.exists("Aerobridge Batch Enrollment", {"batch": batch, "member": member})
		if existing:
			return frappe.get_doc("Aerobridge Batch Enrollment", existing)

		batch_enrollment = frappe.new_doc("Aerobridge Batch Enrollment")
		batch_enrollment.update({"member": member, "batch": batch})
		batch_enrollment.insert()
		self.cleanup_items.append(("Aerobridge Batch Enrollment", batch_enrollment.name))
		return batch_enrollment

	def _add_rating(self, course, member, rating, review_text):
		existing = frappe.db.exists("Aerobridge Course Review", {"course": course, "owner": member})
		if existing:
			return frappe.get_doc("Aerobridge Course Review", existing)

		frappe.session.user = member
		review_doc = frappe.new_doc("Aerobridge Course Review")
		review_doc.update(
			{
				"course": course,
				"rating": rating,
		...
		review_doc.save()
		self.cleanup_items.append(("Aerobridge Course Review", review_doc.name))
		frappe.session.user = "Administrator"
		return review_doc

	def _create_certificate(self, course, member):
		existing = frappe.db.exists("Aerobridge Certificate", {"course": course, "member": member})
		if existing:
			return frappe.get_doc("Aerobridge Certificate", existing)

		certificate = frappe.new_doc("Aerobridge Certificate")
		certificate.update(
			{
				"course": course,
				"member": member,
		...
		certificate.save()
		self.cleanup_items.append(("Aerobridge Certificate", certificate.name))
		return certificate

	def _create_quiz_questions(self):
		questions = []
		for index in range(1, 4):
			question = frappe.new_doc("Aerobridge Question")
			question.update(
				{
			...
			question.save()
			self.cleanup_items.append(("Aerobridge Quiz Question", question.name))
			questions.append(question)
		return questions

	def _create_quiz(self, title="Utility Quiz"):
		existing = frappe.db.exists("Aerobridge Quiz", {"title": title})
		if existing:
			return frappe.get_doc("Aerobridge Quiz", existing)

		quiz = frappe.new_doc("Aerobridge Quiz")
		quiz.update(
			{
				"title": title,
		...
		quiz.save()
		self.cleanup_items.append(("Aerobridge Quiz", quiz.name))
		return quiz

	def _create_assignment(self, title="Utility Assignment"):
		existing = frappe.db.exists("Aerobridge Assignment", {"title": title})
		if existing:
			return frappe.get_doc("Aerobridge Assignment", existing)

		assignment = frappe.new_doc("Aerobridge Assignment")
		assignment.update(
			{
				"title": title,
		...
		assignment.save()
		self.cleanup_items.append(("Aerobridge Assignment", assignment.name))
		return assignment

	def _setup_course_flow(self):
		self.student1 = self._create_user("student1@example.com", "Ashley", "Smith", ["Aerobridge Student"])
		self.student2 = self._create_user("student2@example.com", "John", "Doe", ["Aerobridge Student"])
	...
	def _create_lesson_progress(self, member, course, lesson):
		existing = frappe.db.exists(
			"Aerobridge Course Progress", {"member": member, "course": course, "lesson": lesson}
		)
		if existing:
			return frappe.get_doc("Aerobridge Course Progress", existing)
	...
	def _create_quiz_submission(self, member):
		existing = frappe.db.exists("Aerobridge Quiz Submission", {"quiz": self.quiz.name, "member": member})
		if existing:
			return frappe.get_doc("Aerobridge Quiz Submission", existing)
		submission = frappe.new_doc("Aerobridge Quiz Submission")
		submission.update(
			{
				"quiz": self.quiz.name,
				"member": member,
		...
		submission.insert()
		self.cleanup_items.append(("Aerobridge Quiz Submission", submission.name))
		return submission

	def _create_assignment_submission(self, member):
		existing = frappe.db.exists(
			"Aerobridge Assignment Submission", {"assignment": self.assignment.name, "member": member}
		)
		if existing:
			return frappe.get_doc("Aerobridge Assignment Submission", existing)

		submission = frappe.new_doc("Aerobridge Assignment Submission")
		submission.update(
			{
		...
		submission.insert()
		self.cleanup_items.append(("Aerobridge Assignment Submission", submission.name))
		return submission

	def _create_programming_exercise(self, title="Utility Programming Exercise"):
		existing = frappe.db.exists("Aerobridge Programming Exercise", {"title": title})
		if existing:
			return frappe.get_doc("Aerobridge Programming Exercise", existing)

		programming_exercise = frappe.new_doc("Aerobridge Programming Exercise")
		...
		self.cleanup_items.append(("Aerobridge Programming Exercise", programming_exercise.name))
	...
	def _create_programming_exercise_submission(self, member):
		...
		if existing:
			return frappe.get_doc("Aerobridge Programming Exercise Submission", existing)
		...
		submission.insert()
		self.cleanup_items.append(("Aerobridge Programming Exercise Submission", submission.name))
		return submission
