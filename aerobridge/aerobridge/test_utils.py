...
from aerobridge.aerobridge.doctype.aerobridge_certificate.aerobridge_certificate import is_certified
...
class TestLMSUtils(BaseTestUtils):
	...
	def test_get_instructors(self):
		instructors = get_instructors("Aerobridge Course", self.course.name)
	...
	def test_get_lesson_url(self):
		lessons = get_lessons(self.course.name)
		for lesson in lessons:
			expected_url = get_aerobridge_route(f"courses/{self.course.name}/learn/{lesson.number}")
	...
	def test_rating_validation(self):
		student3 = self._create_user("student3@example.com", "Emily", "Cooper", ["Aerobridge Student"])
		with self.assertRaises(frappe.exceptions.ValidationError):
			frappe.session.user = student3.email
			review = frappe.new_doc("Aerobridge Course Review")
