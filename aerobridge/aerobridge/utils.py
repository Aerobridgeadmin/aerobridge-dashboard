...
from aerobridge.aerobridge.doctype.aerobridge_enrollment.aerobridge_enrollment import update_program_progress
from aerobridge.aerobridge.md import find_macros

RE_SLUG_NOTALLOWED = re.compile("[^a-z0-9]+")
LMS_ROLES = ["Moderator", "Course Creator", "Batch Evaluator", "Aerobridge Student"]


def get_aerobridge_path():
	path = frappe.conf.get("aerobridge_path") or "aerobridge"
	return path.strip("/")


def get_aerobridge_route(path=""):
	base = f"/{get_aerobridge_path()}"
	if not path:
		return base
...
def extend_bootinfo(bootinfo: dict):
	bootinfo["aerobridge_path"] = get_aerobridge_path()
...
def get_membership(course: str, member: str = None):
	...
	if frappe.db.exists("Aerobridge Enrollment", filters):
...
def get_reviews(course: str):
	...
	out_of_ratings = frappe.db.get_all(
		"DocField", {"parent": "Aerobridge Course Review", "fieldtype": "Rating"}, ["options"]
	)
...
def get_lesson_url(course: str, lesson_number: str):
	if not lesson_number:
		return
	return get_aerobridge_route(f"courses/{course}/learn/{lesson_number}")
...
def is_instructor(course: str) -> bool:
	instructors = get_instructors("Aerobridge Course", course)
	for instructor in instructors:
...
def has_student_role(member: str = None):
	return frappe.db.get_value(
		"Has Role",
		{"parent": member or frappe.session.user, "role": "Aerobridge Student"},
		"name",
	)
...
def handle_notifications(doc: Document, method: str):
	...
	if topic.reference_doctype not in ["Course Lesson", "Aerobridge Batch"]:
		return
...
def get_course_details_for_notification(topic: dict):
	...
	course_title = frappe.db.get_value("Aerobridge Course", course, "title")
	instructors = frappe.db.get_all(
		"Course Instructor", {"parent": course, "parenttype": "Aerobridge Course"}, pluck="instructor"
	)
...
def get_batch_details_for_notification(topic: dict):
	users = []
	batch_title = frappe.db.get_value("Aerobridge Batch", topic.reference_docname, "title")
	subject = _("New comment in batch {0}").format(batch_title)
	link = get_aerobridge_route(f"batches/{topic.reference_docname}#discussions")
	instructors = frappe.db.get_all(
		"Course Instructor",
		{"parenttype": "Aerobridge Batch", "parent": topic.reference_docname},
		pluck="instructor",
	)
	students = frappe.db.get_all("Aerobridge Batch Enrollment", {"batch": topic.reference_docname}, pluck="member")
	users += instructors
	users += students
	return subject, link, users
...
def notify_mentions_via_email(doc: Document, topic: dict):
	...
	if topic.reference_doctype == "Aerobridge Batch":
...
def get_course_completion_data():
	all_membership = frappe.db.count("Aerobridge Enrollment")
	completed = frappe.db.count("Aerobridge Enrollment", {"progress": ["like", "%100%"]})
...
def get_evaluator(course: str, batch: str = None):
	...
	if batch:
		...
		evaluator = frappe.db.get_value("Aerobridge Course", course, "evaluator")
	return evaluator


def check_multicurrency(amount: float, currency: str, country: str = None, amount_usd: float = None):
	settings = frappe.get_single("Aerobridge Settings")
...
def apply_gst(amount: float, country: str = None) -> tuple:
	gst_applied = 0
	apply_gst = frappe.db.get_single_value("Aerobridge Settings", "apply_gst")
...
def guest_access_allowed():
	allow_guest_access = frappe.get_cached_value("Aerobridge Settings", None, "allow_guest_access")
...
def get_course_card_details(courses: list) -> list:
	for course in courses:
		course.instructors = get_instructors("Aerobridge Course", course.name)
...
def update_course_filters(filters: dict) -> tuple:
	...
	if filters.get("enrolled"):
		enrolled_courses = frappe.get_all("Aerobridge Enrollment", {"member": frappe.session.user}, pluck="course")
...
def get_course_details(course: str):
	...
	is_course_published = frappe.db.get_value("Aerobridge Course", course, "published")
	...
	course_details.instructors = get_instructors("Aerobridge Course", course_details.name)
...
def get_lesson(course: str, chapter: int, lesson: int) -> dict:
	...
	lesson_details.icon = get_lesson_icon(lesson_details.body, lesson_details.content)
	lesson_details.instructors = get_instructors("Aerobridge Course", course)
...
def get_video_details(lesson_name: str) -> list:
	return frappe.get_all(
		"Aerobridge Video Watch Duration",
...
def get_batch_details(batch: str):
	...
	batch_students = frappe.get_all("Aerobridge Batch Enrollment", {"batch": batch}, pluck="member")
	is_batch_admin = can_modify_batch(batch)
	is_batch_published = frappe.db.get_value("Aerobridge Batch", batch, "published")
	...
	batch_details.instructors = get_instructors("Aerobridge Batch", batch)
	...
	batch_details.assessments = frappe.get_all(
		"Aerobridge Assessment", {"parent": batch}, ["assessment_name", "assessment_type"]
	)
...
def categorize_batches(batches: list) -> dict:
	...
	for batch in batches:
		...
		if frappe.session.user != "Guest":
			if frappe.db.exists("Aerobridge Batch Enrollment", {"member": frappe.session.user, "batch": batch.name}):
...
def get_question_details(question: str) -> dict:
	...
	question_details = frappe.db.get_value("Aerobridge Question", question, fields, as_dict=1)
...
def get_assessments(batch: str) -> list:
	member = frappe.session.user
	is_enrolled = frappe.db.exists("Aerobridge Batch Enrollment", {"batch": batch, "member": member})
	...
	for assessment in assessments:
		if assessment.assessment_type == "Aerobridge Assignment":
			assessment = get_assignment_details(assessment, member)

		elif assessment.assessment_type == "Aerobridge Quiz":
			assessment = get_quiz_details(assessment, member)

		elif assessment.assessment_type == "Aerobridge Programming Exercise":
...
def get_assignment_details(assessment: dict, member: str) -> dict:
	assessment.title = frappe.db.get_value("Aerobridge Assignment", assessment.assessment_name, "title")
	...
	if existing_submission:
		assessment.submission = frappe.db.get_value(
			"Aerobridge Assignment Submission",
			existing_submission,
	...
	assessment.url = get_aerobridge_route(f"assignment-submission/{assessment.assessment_name}/{submission_name}")
...
def get_quiz_details(assessment: dict, member: str) -> dict:
	assessment_details = frappe.db.get_value(
		"Aerobridge Quiz", assessment.assessment_name, ["title", "passing_percentage"], as_dict=1
	)
	assessment.title = assessment_details.title

	existing_submission = frappe.get_all(
		"Aerobridge Quiz Submission",
		{
			"member": member,
...
def get_exercise_details(assessment: dict, member: str) -> dict:
	assessment.title = frappe.db.get_value("Aerobridge Programming Exercise", assessment.assessment_name, "title")
	filters = {"member": member, "exercise": assessment.assessment_name}

	if frappe.db.exists("Aerobridge Programming Exercise Submission", filters):
		assessment.submission = frappe.db.get_value(
			"Aerobridge Programming Exercise Submission",
			filters,
			["name", "status"],
			as_dict=True,
		)
...
def get_course_completion_stats(batch: str) -> list:
	...
	BatchEnrollment = frappe.qb.DocType("Aerobridge Batch Enrollment")
	Enrollment = frappe.qb.DocType("Aerobridge Enrollment")
...
def get_assignment_pass_stats(batch: str) -> list:
	...
	Assessment = frappe.qb.DocType("Aerobridge Assessment")
	Assignment = frappe.qb.DocType("Aerobridge Assignment")
	BatchEnrollment = frappe.qb.DocType("Aerobridge Batch Enrollment")
	Submission = frappe.qb.DocType("Aerobridge Assignment Submission")
	...
	rows = (
		...
		.where((Assessment.parent == batch) & (Assessment.assessment_type == "Aerobridge Assignment"))
...
def get_quiz_pass_stats(batch: str) -> list:
	...
	Assessment = frappe.qb.DocType("Aerobridge Assessment")
	Quiz = frappe.qb.DocType("Aerobridge Quiz")
	BatchEnrollment = frappe.qb.DocType("Aerobridge Batch Enrollment")
	Submission = frappe.qb.DocType("Aerobridge Quiz Submission")
	...
	rows = (
		...
		.where((Assessment.parent == batch) & (Assessment.assessment_type == "Aerobridge Quiz"))
...
def get_batch_chart_data(batch: str) -> list:
	...
	if not frappe.db.exists("Aerobridge Batch", batch):
...
def calculate_course_progress(batch_courses: list, details: dict):
	...
	for course in batch_courses:
		progress = (
			frappe.db.get_value(
				"Aerobridge Enrollment", {"course": course.course, "member": details.email}, "progress"
			)
			or 0
		)
...
def get_assessment_meta(assessment_type: str):
	if assessment_type == "Aerobridge Assignment":
		doctype = "Aerobridge Assignment Submission"
		docfield = "assignment"
		fields = ["status"]
		not_attempted = "Not Attempted"
	elif assessment_type == "Aerobridge Quiz":
		doctype = "Aerobridge Quiz Submission"
		docfield = "quiz"
		fields = ["percentage"]
		not_attempted = 0
	elif assessment_type == "Aerobridge Programming Exercise":
...
def get_assessment_attempt_details(
	...
	if assessment_type == "Aerobridge Quiz":
		result = "Failed"
		passing_percentage = frappe.db.get_value("Aerobridge Quiz", assessment, "passing_percentage")
	...
	return frappe._dict(
		{
			"status": attempt_details.percentage if assessment_type == "Aerobridge Quiz" else attempt_details.status,
			"result": result,
...
def can_access_topic(doctype: str, docname: str) -> bool:
	...
	if doctype == "Course Lesson":
		...
		is_student = frappe.db.exists("Aerobridge Enrollment", {"course": course, "member": frappe.session.user})
		if not is_student and not can_modify_course(course):
			return False
	elif doctype == "Aerobridge Batch":
		is_student = frappe.db.exists(
			"Aerobridge Batch Enrollment", {"batch": docname, "member": frappe.session.user}
		)
...
def get_order_summary(doctype: str, docname: str, coupon: str = None, country: str = None):
	details = get_paid_course_details(docname) if doctype == "Aerobridge Course" else get_paid_batch_details(docname)
...
def apply_coupon(doctype: str, docname: str, code: str, base_amount: float):
	coupon_name = frappe.db.exists("Aerobridge Coupon", {"code": code, "enabled": 1})
	if not coupon_name:
...
def validate_coupon_applicability(doctype: str, docname: str, coupon_name: str):
	applicable_item = frappe.db.exists(
		"Aerobridge Coupon Item", {"parent": coupon_name, "reference_doctype": doctype, "reference_name": docname}
	)
	if not applicable_item:
		frappe.throw(
			_("This coupon is not applicable to this {0}.").format(
				"Course" if doctype == "Aerobridge Course" else "Batch"
			)
		)
...
def get_roles(name: str) -> dict:
	...
	return {
		...
		"aerobridge_student": has_student_role(name),
	}


def publish_notifications(doc: Document, method: str):
	frappe.publish_realtime("publish_aerobridge_notifications", user=doc.for_user, after_commit=True)
...
def update_payment_record(doctype: str, docname: str):
	...
	if len(request):
		...
		elif doctype == "Aerobridge Course":
...
def get_payment_doc(payment_name: str) -> dict:
	return frappe.db.get_value(
		"Aerobridge Payment", payment_name, ["name", "coupon", "payment_for_certificate"], as_dict=True
	)
...
def update_coupon_redemption(payment_doc: dict):
	if payment_doc.coupon:
		redemption_count = frappe.db.get_value("Aerobridge Coupon", payment_doc.coupon, "redemption_count") or 0
...
def enroll_in_course(course: str, payment_name: str):
	if not frappe.db.exists("Aerobridge Enrollment", {"member": frappe.session.user, "course": course}):
		enrollment = frappe.new_doc("Aerobridge Enrollment")
		payment = frappe.db.get_value("Aerobridge Payment", payment_name, ["name", "source"], as_dict=True)
...
def get_payment_details(payment_name: str) -> dict:
	...
	if payment_name:
		payment_doc = frappe.db.get_value(
			"Aerobridge Payment", payment_name, ["name", "source", "payment_received"], as_dict=True
		)
	return payment_doc


def create_enrollment(batch: str, payment_doc: dict = None):
	new_student = frappe.new_doc("Aerobridge Batch Enrollment")
	new_student.update(
		{
			"member": frappe.session.user,
			"batch": batch,
...
def get_programs():
	...
	enrolled_programs = frappe.get_all(
		"Aerobridge Program Member", {"member": frappe.session.user}, ["parent as name", "progress"]
	)
	for program in enrolled_programs:
		program.update(
			frappe.db.get_value(
				"Aerobridge Program", program.name, ["name", "course_count", "member_count"], as_dict=True
			)
		)
...
def get_program_details(program_name: str) -> dict:
	...
	is_published = frappe.db.get_value("Aerobridge Program", program_name, "published")
	is_member = frappe.db.exists(
		"Aerobridge Program Member", {"parent": program_name, "member": frappe.session.user}
	)
	...
	program_courses = frappe.get_all(
		"Aerobridge Program Course", {"parent": program_name}, ["course"], order_by="idx"
	)
...
def enroll_in_program(program: str):
	...
	if not frappe.db.exists("Aerobridge Program Member", {"parent": program, "member": frappe.session.user}):
		program_member = frappe.new_doc("Aerobridge Program Member")
...
def validate_program_enrollment(program: str):
	published = frappe.db.get_value("Aerobridge Program", program, "published")
	if not published:
...
def get_batches(filters: dict = None, start: int = 0, order_by: str = "start_date"):
	...
	if filters.get("enrolled"):
		enrolled_batches = frappe.get_all(
			"Aerobridge Batch Enrollment", {"member": frappe.session.user}, pluck="batch"
		)
...
def get_batch_card_details(batches: list) -> list:
	for batch in batches:
		batch.instructors = get_instructors("Aerobridge Batch", batch.name)
		students_count = frappe.db.count("Aerobridge Batch Enrollment", {"batch": batch.name})
...
def persona_captured():
	frappe.db.set_single_value("Aerobridge Settings", "persona_captured", 1)
...
def validate_discussion_reply(doc: Document, method: str):
	...
	elif topic.reference_doctype == "Aerobridge Batch":
...
def validate_course_access(lesson: str):
	...
	enrollment_exists = frappe.db.exists("Aerobridge Enrollment", {"member": frappe.session.user, "course": course})
...
def validate_batch_access(batch: str):
	...
	enrollment_exists = frappe.db.exists(
		"Aerobridge Batch Enrollment", {"member": frappe.session.user, "batch": batch}
	)
	if not enrollment_exists:
...
def can_modify_course(course: str) -> bool:
	is_instructor = frappe.db.exists(
		"Course Instructor",
		{"instructor": frappe.session.user, "parent": course, "parenttype": "Aerobridge Course"},
	)
...
def has_aerobridge_role():
	roles = frappe.get_roles()
	aerobridge_roles = set(LMS_ROLES)
	user_roles = set(roles)
	return not aerobridge_roles.isdisjoint(user_roles)
...
def recalculate_course_progress(course: str, member: str):
	...
	frappe.db.set_value("Aerobridge Enrollment", membership, "progress", progress)
