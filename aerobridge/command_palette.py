...
def search_sqlite(query: str):
	from aerobridge.sqlite import LearningSearch, LearningSearchIndexMissingError
...
def get_grouped_results(result):
	...
	for r in result["results"]:
		doctype = r["doctype"]
		if doctype == "Aerobridge Course" and can_access_course(r, roles):
			r["author_info"] = get_instructor_info(doctype, r)
			groups.setdefault("Courses", []).append(r)
		elif doctype == "Aerobridge Batch" and can_access_batch(r, roles):
			r["author_info"] = get_instructor_info(doctype, r)
