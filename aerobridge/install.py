...
from aerobridge.aerobridge.api import give_discussions_permission
...
def after_sync():
	create_aerobridge_roles()
	set_default_certificate_print_format()
	give_aerobridge_roles_to_admin()


def before_uninstall():
	delete_custom_fields()
	delete_aerobridge_roles()


def create_aerobridge_roles():
...
def create_aerobridge_student_role():
	if frappe.db.exists("Role", "Aerobridge Student"):
		frappe.db.set_value("Role", "Aerobridge Student", "desk_access", 0)
	else:
		role = frappe.new_doc("Role")
		role.update(
			{
				"role_name": "Aerobridge Student",
				"home_page": "",
				"desk_access": 0,
			}
		)
...
def create_batch_source():
	...
	for source in sources:
		if not frappe.db.exists("Aerobridge Source", source):
			doc = frappe.new_doc("Aerobridge Source")
			doc.source = source
			doc.save()


def give_aerobridge_roles_to_admin():
...
def delete_aerobridge_roles():
	roles = ["Course Creator", "Moderator", "Batch Evaluator", "Aerobridge Student"]
	for role in roles:
