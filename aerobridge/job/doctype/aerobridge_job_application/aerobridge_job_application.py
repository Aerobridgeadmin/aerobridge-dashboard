...
class LMSJobApplication(Document):
	...
	def after_insert(self):
		...
		if job_owner:
			frappe.share.add_docshare("Aerobridge Job Application", self.name, job_owner, read=1)
	...
	def validate_duplicate(self):
		if frappe.db.exists("Aerobridge Job Application", {"job": self.job, "user": self.user}):
