...
class LiveCodeExtension(PageExtension):
	def render_header(self):
		livecode_url = frappe.get_value("Aerobridge Settings", None, "livecode_url")
...
def quiz_renderer(quiz_name):
	...
	for question in questions:
		details = frappe.db.get_value("Aerobridge Question", question.question, fields, as_dict=1)
		details["marks"] = question.marks
		quiz.questions.append(details)

	no_of_attempts = frappe.db.count("Aerobridge Quiz Submission", {"owner": frappe.session.user, "quiz": quiz_name})
