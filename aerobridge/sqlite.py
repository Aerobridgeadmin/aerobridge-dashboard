...
class LearningSearch(SQLiteSearch):
	...
	DOCTYPE_FIELDS = {
		"Aerobridge Course": COURSE_FIELDS,
		"Aerobridge Batch": BATCH_FIELDS,
	...
	def get_instructor_details(self, doc, document):
		...
		fields = self.COURSE_FIELDS if doc.parenttype == "Aerobridge Course" else self.BATCH_FIELDS
		...
		if details:
			...
			if doc.parenttype == "Aerobridge Course":
				document["published_on"] = details.get("published_on")
			elif doc.parenttype == "Aerobridge Batch":
