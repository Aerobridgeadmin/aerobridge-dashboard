          ...
          <div key={doc.id} className="rounded-xl border bg-card p-4">
            <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  {doc.description && doc.documentType === "time_doctor_report" ? (
                    <details className="mt-0.5 group">
                      <summary className="text-[10px] text-primary cursor-pointer hover:underline">View Time Doctor Report</summary>
                      <pre className="mt-1.5 max-h-48 overflow-auto rounded-md bg-muted/50 p-2 text-[10px] text-muted-foreground whitespace-pre-wrap font-mono">{doc.description}</pre>
                    </details>
                  ) : doc.description ? (
              ...
              {(doc.fileUrl || doc.filePath) && (
                <a href={`/api/documents/view?id=${doc.id}`} target="_blank" rel="noopener noreferrer"
