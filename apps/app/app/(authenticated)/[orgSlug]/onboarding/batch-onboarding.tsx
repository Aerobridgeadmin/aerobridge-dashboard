  ...
  const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to create batch session." });
      }
    });
  };
  ...
  const handleAddEmployees = (batchId: string, employeeIds: string[]) => {
    startTransition(async () => {
      try {
        ...
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to add employees to batch." });
      }
    });
  };

  const handleCancel = (batchId: string) => {
    setConfirmCancelBatchId(batchId);
  };

  const executeCancel = (batchId: string) => {
    setConfirmCancelBatchId(null);
    startTransition(async () => {
      try {
        await updateBatchSession(batchId, { status: "cancelled" });
      } catch (err) {
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to cancel batch." });
      }
    });
  };
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {session.zoomMeetingDate && (
                    <p className="mt-2 text-sm">Zoom: {new Date(session.zoomMeetingDate as any).toLocaleString("en-US", { timeZone: "America/Los_Angeles", weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })} PT ({session.zoomDuration} min)</p>
                  )}
                  {session.zoomJoinUrl && (
              ...
              {session.onboardingSessions.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {session.onboardingSessions.map((os: any) => (
                      <Link key={os.id} href={`/${orgSlug}/employees/${os.employee.id}`} className="inline-flex items-center rounded-full border px-3 py-1 text-sm hover:bg-accent">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
                    {session.zoomMeetingDate && (
                      <span>{new Date(session.zoomMeetingDate as any).toLocaleString("en-US", { timeZone: "America/Los_Angeles", weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })} PT</span>
                    )}
              ...
              {session.onboardingSessions.length > 0 && (
                <div className="mt-4 border-t pt-3">
                  <div className="flex flex-wrap gap-2">
                    {session.onboardingSessions.map((os: any) => (
                      <Link key={os.id} href={`/${orgSlug}/employees/${os.employee.id}`} className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs hover:bg-accent">
      ...
      {confirmCancelBatchId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-xl border bg-card p-6 shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-200" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold">Cancel Onboarding Session</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Cancel this onboarding session? This will mark it as cancelled.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setConfirmCancelBatchId(null)} disabled={isPending} className="h-9 rounded-md border px-4 text-sm hover:bg-accent disabled:opacity-50">Go Back</button>
              <button
                type="button"
                onClick={() => executeCancel(confirmCancelBatchId)}
                disabled={isPending}
    ...
    <div className="px-4 py-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-white dark:bg-white/90 p-0.5">
      ...
      {msg && (
        <div className={`mt-2 ml-11 rounded px-2 py-1 text-xs ${msg.startsWith("Error") ? "bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300" : "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300"}`}>
          {msg}
        </div>
      )}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    onRefresh();
                  } catch (e) { const m = e instanceof Error ? e.message : "Failed"; setMsg(`Error: ${m}`); showError({ title: "Error", message: m }); }
