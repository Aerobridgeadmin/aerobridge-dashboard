...
export function PendingHiresList({ hires }: { hires: PendingHire[] }) {
  ...
  const handleApprove = (id: string) => {
    startTransition(async () => {
      try {
        const result = await approvePendingHire(id);
        if (result && typeof result === "object" && "employeeId" in result) {
          showError({ title: "Hire Approved", message: "Contractor created. View their profile in the Contractors directory." });
        }
      } catch (err) {
        showError({ title: "Approval Failed", message: err instanceof Error ? err.message : "Failed to approve hire." });
      }
    });
  };
  ...
  const confirmReject = () => {
    ...
    startTransition(async () => {
      try {
        ...
        showError({ title: "Rejection Failed", message: err instanceof Error ? err.message : "Failed to reject hire." });
      }
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
              ...
              startTransition(async () => {
                try {
                  ...
                  if (result.error) {
                    ...
                    const parts = [`Synced ${result.synced} new hired candidates`];
                    if ((result.skipped ?? 0) > 0) parts.push(`${result.skipped} already imported`);
                    parts.push(`from ${result.total} total scanned`);
          ...
          >
            {isPending ? "Syncing..." : "Sync from RecruitCRM"}
          </button>
          {pending.length > 0 && (
      ...
      {syncMessage && (
        <div className={`rounded-lg border px-4 py-3 text-sm ${syncMessage.startsWith("Sync failed") ? "border-red-200 bg-red-50 text-red-700 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300" : "border-green-200 bg-green-50 text-green-700 dark:border-green-800 dark:bg-green-950/30 dark:text-green-300"}`}>
          {syncMessage}
          <button onClick={() => setSyncMessage(null)} className="ml-3 text-xs underline">Dismiss</button>
        </div>
      )}
      ...
      {pending.length === 0 && (
        <div className="rounded-xl border bg-card p-8 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Webhook URL: <code className="rounded bg-muted px-1.5 py-0.5">{typeof window !== "undefined" ? window.location.origin : "https://hriq.remoteleverage.com"}/api/webhooks/recruitcrm</code></p>
        </div>
      )}
              ...
              <div key={hire.id} className="flex items-center justify-between rounded-lg border p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {hire.created_employee_id && (
                    <Link href={`/${orgSlug}/employees/${hire.created_employee_id}`} className="text-xs text-primary hover:underline">View Contractor</Link>
                  )}
