...
const SecurityPage = async () => {
  ...
  return (
    <>
      <Header page="Security" pages={["RL Internal", "Settings"]} />
          ...
          <div className="rounded-xl border bg-card p-6">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs ${process.env.ARCJET_KEY ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}>
                  {process.env.ARCJET_KEY ? "Enabled" : "Disabled"}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Error Tracking (Sentry)</span>
                <span className={`rounded-full px-2 py-0.5 text-xs ${process.env.SENTRY_DSN ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}>
