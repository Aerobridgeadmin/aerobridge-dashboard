  ...
  const handleApprove = (id: string) => {
    startTransition(async () => {
      try {
        ...
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to approve request." });
      }
    });
  };
  ...
  const confirmReject = () => {
    ...
    startTransition(async () => {
      try {
        ...
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to reject request." });
      }
    });
  };
  ...
  const handleCreatePolicy = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        showError({ title: "Error", message: err instanceof Error ? err.message : "Failed to create policy." });
      }
    });
  };
              ...
              <div key={r.id} className="flex items-center justify-between rounded-lg border p-4">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {r.policy.name} · {shortDate(r.startDate as any)} – {shortDate(r.endDate as any)} · {Number(r.totalDays)} days
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleReject(r.id)} disabled={isPending} className="h-8 rounded-md border px-3 text-xs font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-950">Reject</button>
                </div>
              </div>
            ))}
          </div>
