...
const UsersPage = async ({ searchParams }: UsersPageProps) => {
  ...
  const userIds = users.map((u) => u.supabaseUserId);
  ...
  return (
    <>
      <Header page="Users" pages={["RL Admin", "Settings"]} />
      <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">User Management ({total})</h2>
          <form action="/rl/settings/users" method="GET" className="flex gap-2">
            <input
              type="text"
              name="q"
              defaultValue={q ?? ""}
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className={`rounded-full px-2 py-0.5 text-xs ${u.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}>
                        {u.isActive ? "Active" : "Inactive"}
                      </span>
                      <span className="rounded-full bg-muted px-2 py-0.5 text-xs">{u.loginCount} logins</span>
                      <span className="rounded-full bg-muted px-2 py-0.5 text-xs">Joined {new Date(u.createdAt).toLocaleDateString()}</span>
                    </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 pt-2">
            {page > 1 && (
              <Link
                href={`/rl/settings/users?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
