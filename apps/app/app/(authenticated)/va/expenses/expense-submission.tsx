            ...
            <div className="mt-2 space-y-2">
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-4 gap-2">
                  <select value={item.category} onChange={(e) => { const n = [...items]; n[i].category = e.target.value; setItems(n); }} className="h-9 rounded-md border border-input bg-background px-2 text-sm">
                  ...
                  <input type="date" value={item.date} onChange={(e) => { const n = [...items]; n[i].date = e.target.value; setItems(n); }} required className="h-9 rounded-md border border-input bg-background px-2 text-sm" />
                </div>
              ))}
            </div>
          </div>
        ...
        {reports.map((r) => (
          <div key={r.id} className="flex items-center justify-between rounded-xl border bg-card p-4">
            <div>
              ...
              <div className="text-sm text-muted-foreground">${Number(r.totalAmount).toLocaleString()} · {r._count.items} items · {new Date(r.createdAt).toLocaleDateString()}</div>
            </div>
