...
export function WiseSetupForm({ token, contractorName, prefill }: Props) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null);
    try {
      const { submitWiseSetup } = await import("@/app/actions/hriq/wise-setup");
      const finalBankName = form.bankName === "Other" ? bankOther : form.bankName;
      const result = await submitWiseSetup(token, {
      ...
      <div className="w-full max-w-lg">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-gray-400">
            Hi {contractorName}! Please fill in your banking information below so we can pay you via Wise.
          </p>
        </div>
        ...
        <p className="mt-4 text-center text-[11px] text-gray-600">
          Your information is encrypted and securely stored. Remote Leverage uses Wise for safe international payments.
        </p>
      </div>
    </div>
  );
