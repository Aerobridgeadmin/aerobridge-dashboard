      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          {batches.map((batch, i) => (
            <div
              key={batch.id}
              className="card group animate-slide-up p-6"
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`badge text-[10px] ${statusStyles[batch.status]}`}>
                      {batch.status}
                    </span>
                  </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-4">
                <div>
                  <p className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-surface-400">
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-surface-100">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      batch.student_count / batch.max_students > 0.9
                        ? 'bg-rose-500'
                        : batch.student_count / batch.max_students > 0.7
                        ? 'bg-amber-500'
                        : 'bg-brand-500'
                    }`}
                    style={{ width: `${(batch.student_count / batch.max_students) * 100}%` }}
                  ></div>
                </div>
                <p className="mt-1 text-[10px] text-surface-400">
                  {Math.round((batch.student_count / batch.max_students) * 100)}% capacity
                </p>
              </div>
            </div>
          ))}
