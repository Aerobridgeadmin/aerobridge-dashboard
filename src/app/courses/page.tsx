...
import { Plus, BookOpen, Users, Layers2, MoreVertical, Search } from 'lucide-react'
            ...
            >
              ...
              <div className="relative h-36 bg-gradient-to-br from-brand-600 via-brand-500 to-brand-400 p-5">
                ...
                <div className="relative z-10">
                  <span className={`badge ${categoryColors[course.category] || 'badge-blue'} !bg-white/20 !text-white backdrop-blur-sm`}>
                    {course.category}
                  </span>
                  <h3 className="mt-3 font-display text-xl leading-tight text-white">{course.title}</h3>
                </div>
                {!course.published && (
                  <div className="absolute right-3 top-3 rounded-full bg-surface-900/60 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white backdrop-blur-sm">
                    Draft
                  </div>
                )}
              </div>
